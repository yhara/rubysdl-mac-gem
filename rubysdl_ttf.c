...
static VALUE TT_s_init(VALUE klass)
{
  rb_secure(4);
  if(TTF_Init() == -1)
...
{
  return rb_str_new2((const char *)TTF_FontFaceFamilyName(Get_TTF_Font(self)));
}

static VALUE Font_styleName(VALUE self)
{
  return rb_str_new2((const char *)TTF_FontFaceStyleName(Get_TTF_Font(self)));
}
...
{
  VALUE mTT = rb_define_module_under(mSDL,"TT");
  cTTFFont = rb_define_class_under(mTT, "Font", rb_cObject);
                                         
  rb_undef_alloc_func(cTTFFont);
  
  rb_define_singleton_method(mTT,"init",TT_s_init,0);
  ...
  rb_define_const(mTT,"STYLE_NORMAL",UINT2NUM(TTF_STYLE_NORMAL));
  rb_define_const(mTT,"STYLE_BOLD",UINT2NUM(TTF_STYLE_BOLD));
  rb_define_const(mTT,"STYLE_ITALIC",UINT2NUM(TTF_STYLE_ITALIC));
  rb_define_const(mTT,"STYLE_UNDERLINE",UINT2NUM(TTF_STYLE_UNDERLINE));
}
